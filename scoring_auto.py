...
def insert_cluster_scores_batchwise(username, password, db_config, df_scores, landing_zone_df, batch_size):
    ...
    if landing_zone_df.empty:
        print("Landing zone DataFrame is empty. Cannot filter accounts for insertion.")
    cluster_mdb_major_version DOUBLE, instance_size VARCHAR, is_auto_expand_storage BOOLEAN, # Already used db_config
    is_auto_scaling_compute_enabled BOOLEAN, is_auto_scaling_compute_scaledown_enabled BOOLEAN, # for connection in create_table
    is_backup_selected BOOLEAN, is_sharding BOOLEAN, region_count INTEGER, shard_count INTEGER, # and insert_... functions
